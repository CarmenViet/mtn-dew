...
def handle_jump_press(char_physics, can_jump):
    # Handle up button
    keys_pressed=pygame.key.get_pressed()
    if keys_pressed[pygame.K_UP] and can_jump:
...
def check_if_touching_obstacle(char_physics, obstacle_list, check_distance, ground_level):
    obstacle_distances = check_for_object(char_physics, obstacle_list, check_distance, ground_level)
    for distance in obstacle_distances.values():
        if distance != None:
            pygame.quit()
